...
want DEVTMPFS                        y     "/dev"
want EROFS_FS                        y     "rootfs lower (mount-instead-of-extract materialization; erofs blob + no driver = boot ENODEV)"
want EROFS_FS_XATTR                  y     "image xattrs"
want EROFS_FS_SECURITY               y     "security.capability on image files"
want EROFS_FS_ZIP                    not-y "uncompressed images only — ZIP drags LZ4/XZ decompressors into the kernel for nothing"
