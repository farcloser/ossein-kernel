...
release-kernel tag:
    ...
    # --- release notes: every build input, so the artifact is reproducible from the release ---
    cfg_sha="$(shasum -a 256 "{{ kernel_config }}" | cut -d' ' -f1)"
    golden_sha="$(shasum -a 256 kernel/config/kernel-golden | cut -d' ' -f1)"
    {
        echo "# ${tag}"
        echo
        echo "\`${upstream}-{{ kernel_localversion }}\` guest kernel for Apple Virtualization.framework (arm64)."
        echo
        echo "## Verify"
        echo '```'
        echo "cosign verify-blob --bundle SHA256SUMS.cosign.bundle \\"
        echo "  --certificate-identity-regexp '<pin the maintainer identity>' \\"
        echo "  --certificate-oidc-issuer-regexp '<pin the OIDC issuer>' SHA256SUMS"
        ...
        echo "- config fragment: {{ kernel_config }} (sha256 ${cfg_sha})"
        echo "- golden config: sha256 ${golden_sha}"
        echo
        echo "## Patches"
        for p in {{ kernel_patches }}/*.patch; do
            [ -e "$p" ] || { echo "- (none)"; break; }
            echo "- $(basename "$p") (sha256 $(shasum -a 256 "$p" | cut -d' ' -f1))"
        done
    } > "$rel/NOTES.md"
